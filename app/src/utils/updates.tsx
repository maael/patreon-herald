...
export async function getLatestDifferentRelease() {
  try {
    ...
    const data = await fetch(`https://patreon-herald.mael.tech/api/version`).then((res) => res.json())
    const updateInfo = {
      name: `v${data.version}`,
...
export async function checkForUpdate() {
  try {
    const manifest = await Neutralino.updater.checkForUpdates(`https://patreon-herald.mael.tech/api/version`)
